# Panel with parameters to paint roughness brush and color textures
# Brush color is used for roughness intensity
...
# Brush panel shows and sets default brush values (min roughness, small size)
func show_brush_panel():
	$VBoxContainer/BrushPanel.show()
	$VBoxContainer/BucketPanel.hide()
	$VBoxContainer/EraserPanel.hide()
	$VBoxContainer/BrushPanel/VBoxContainer2/ValueSlider.value = 0.0
	$VBoxContainer/BrushPanel/VBoxContainer3/SizeSlider.value = 0.1
	_on_Brush_ValueSlider_value_changed(0.0)
	_on_Brush_SizeSlider_value_changed(0.1)

# Bucket panel shows and sets default brush values (min roughness)
func show_bucket_panel():
	$VBoxContainer/BrushPanel.hide()
	$VBoxContainer/BucketPanel.show()
	$VBoxContainer/EraserPanel.hide()
	
	$VBoxContainer/BucketPanel/VBoxContainer2/ValueSlider.value = 0.0
	_on_Bucket_ValueSlider_value_changed(0.0)

# Eraser panel shows and sets default brush values (small size)
...
func _on_Eraser_SizeSlider_value_changed(size: float) -> void:
	# Use white on top as a way to "erase"
